...
:root{
  ...
  --white:#FDFCFD;--off:#F7F5F9;--ink:#1A1520;--mid:#3D3845;
  --light:#6B6270;--border:#E5E0EA;--red:#C0392B;
}
...
.hero-sub{font-size:16px;color:var(--mid);max-width:400px;line-height:1.75;margin-bottom:40px;}
...
.stat-l{font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:var(--light);margin-top:4px;}
...
.card-addr{font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:var(--light);margin-bottom:6px;}
...
.feat{display:flex;align-items:center;gap:5px;font-size:13px;color:var(--mid);}
.card-foot{display:flex;align-items:center;justify-content:space-between;}
.card-price{font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:400;line-height:1;}
...
.lp-desc{font-size:16px;color:var(--mid);line-height:1.85;max-width:580px;}
...
.about-body{font-size:16px;color:var(--mid);line-height:1.85;margin-bottom:24px;}
...
/* PURPLE BAND */
.purple-band{background:var(--purple);padding:64px 72px;display:flex;align-items:center;justify-content:space-between;gap:48px;margin-left:0;margin-right:0;width:100%;}
.pb-left{flex:1;}
...
.ci-lbl{font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--mid);margin-bottom:3px;}
...
.fg label{display:block;font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--ink);margin-bottom:6px;font-weight:500;}
.fg input,.fg textarea,.fg select{width:100%;padding:10px 14px;border:1px solid var(--border);background:var(--off);font-family:'DM Sans',sans-serif;font-size:14px;color:var(--ink);outline:none;transition:border-color .2s;appearance:none;}
...
.ft-brand{font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:300;color:white;}
.ft-brand span{color:#C49DDD;}
.ft-desc{font-size:13px;color:#aaa;line-height:1.75;max-width:260px;margin-bottom:20px;}
.ft-col-t{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:white;margin-bottom:16px;}
.ft-links{list-style:none;}
.ft-links li{margin-bottom:8px;}
.ft-links a{font-size:13px;color:#999;text-decoration:none;transition:color .2s;}
.ft-links a:hover{color:#C49DDD;}
.ft-bot{display:flex;justify-content:space-between;}
.ft-copy{font-size:12px;color:#666;}
.ft-city{font-family:'Cormorant Garamond',serif;font-style:italic;font-size:13px;color:#666;}
...
const BLANK = { id:null, tipo:"Departamento", etiqueta:"Venta", tag:"", titulo:"", direccion:"", ciudad:"Mar del Plata", precio:"", m2:"", m2lote:"", dorm:"", banos:"", cochera:"", descripcion:"", imagen:"", url:"", lazo:"", standby:false, destacado:false };
      ...
      <div className="lp-body">
        <div className="lp-main">
          ...
          <div className="lp-feats">
            ...
            {p.cochera&& <div className="lp-feat"><div className="lp-feat-lbl">{parseInt(p.cochera)===1?"Cochera":"Cocheras"}</div><div className="lp-feat-val">{p.cochera}</div></div>}
          </div>
          ...
          <div style={{marginTop:36}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div style={{position:"relative",height:240,overflow:"hidden",border:"1px solid var(--border)"}}>
                <div style={{position:"absolute",top:8,left:8,zIndex:2,background:"white",padding:"3px 8px",fontSize:9,letterSpacing:".1em",textTransform:"uppercase",color:"var(--ink)"}}>Mapa</div>
                ...
                <a href={`https://maps.google.com/?q=${mapQuery}`} target="_blank" rel="noreferrer"
                  style={{position:"absolute",bottom:8,right:8,background:"white",border:"1px solid var(--border)",padding:"4px 8px",fontSize:9,letterSpacing:".08em",textTransform:"uppercase",textDecoration:"none",color:"var(--ink)",display:"flex",alignItems:"center",gap:4}}>
              ...
              <div style={{position:"relative",height:240,overflow:"hidden",border:"1px solid var(--border)"}}>
                <div style={{position:"absolute",top:8,left:8,zIndex:2,background:"white",padding:"3px 8px",fontSize:9,letterSpacing:".1em",textTransform:"uppercase",color:"var(--ink)"}}>Street View</div>
                <iframe title="streetview" width="100%" height="100%" style={{border:0,display:"block"}} loading="lazy"
                  src={`https://www.google.com/maps/embed/v1/streetview?key=AIzaSyD-9tSrke72PouQMnMX-a7eZSW0jkFMBWY&location=${mapQuery}&heading=180&pitch=0&fov=90`}/>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{border:"1px solid var(--border)",padding:"16px 18px",background:"white",width:"100%"}}>
            ...
            <div style={{fontFamily:"Cormorant Garamond,serif",fontSize:28,fontWeight:300,lineHeight:1.1,color:"var(--ink)"}}>
              {p.precio
                ...
                : <span style={{fontSize:20,fontFamily:"DM Sans,sans-serif",fontWeight:300,color:"var(--mid)"}}>Consulte</span>
              }
            </div>
          </div>
        </div>
        ...
        <div className="ft">
          <div>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
              ...
              <LogoIcon variant="white" size={0.31} opacity={0.85}/>
            </div>
          ...
          <div>
            ...
            <ul className="ft-links">
              <li><a href="#propiedades" onClick={e=>{e.preventDefault();document.getElementById("propiedades")?.scrollIntoView({behavior:"smooth"})}}>Departamentos</a></li>
              <li><a href="#propiedades" onClick={e=>{e.preventDefault();document.getElementById("propiedades")?.scrollIntoView({behavior:"smooth"})}}>Casas / Chalets</a></li>
              <li><a href="#propiedades" onClick={e=>{e.preventDefault();document.getElementById("propiedades")?.scrollIntoView({behavior:"smooth"})}}>PH</a></li>
              <li><a href="#propiedades" onClick={e=>{e.preventDefault();document.getElementById("propiedades")?.scrollIntoView({behavior:"smooth"})}}>Terrenos</a></li>
              <li><a href="#propiedades" onClick={e=>{e.preventDefault();document.getElementById("propiedades")?.scrollIntoView({behavior:"smooth"})}}>Hoteles</a></li>
            </ul>
          </div>
        </div>
...
function ContactForm({ prefill="" }) {
  ...
  const send = async () => {
    if (!nombre || !email) { alert("Por favor completá al menos nombre y email."); return; }
    setStatus("sending");
    try {
      const res = await fetch("/", {
        ...
        body: encode({
          "form-name": "contacto",
          nombre, apellido, email, tel, interes,
          mensaje: msg,
        }),
      });
      if (res.ok) { setStatus("sent"); setNombre(""); setApellido(""); setEmail(""); setTel(""); setMsg(""); }
      else setStatus("error");
    } catch { setStatus("error"); }
  };

  if (status === "sent") return (
    <div className="contact-form" style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:320,gap:16,textAlign:"center"}}>
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#7B35A0" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><polyline points="9,12 11.5,14.5 15.5,9.5"/></svg>
      <div style={{fontFamily:"Cormorant Garamond,serif",fontSize:26,fontWeight:400}}>¡Mensaje enviado!</div>
      <div style={{fontSize:14,color:"var(--mid)"}}>Nos pondremos en contacto a la brevedad.</div>
      <button className="submit" style={{maxWidth:200}} onClick={()=>setStatus("idle")}>Enviar otro</button>
    </div>
  );
  ...
  return (
    <div className="contact-form">
      ...
      <div className="fr">
        <div className="fg"><label>Nombre</label><input value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Nombre"/></div>
        <div className="fg"><label>Apellido</label><input value={apellido} onChange={e=>setApellido(e.target.value)} placeholder="Apellido"/></div>
      </div>
      <div className="fg"><label>Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="su@email.com"/></div>
      ...
      <div className="fg"><label>Me interesa</label>
        <div style={{position:"relative"}}>
          <select value={interes} onChange={e=>setInteres(e.target.value)} style={{paddingRight:36}}>
          ...
          <svg style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",pointerEvents:"none",color:"var(--mid)"}} width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
        </div>
      </div>
      ...
      {status === "error" && <div style={{fontSize:12,color:"var(--red)",marginBottom:8}}>Hubo un error. Por favor intentá de nuevo.</div>}
      <button className="submit" onClick={send} disabled={status==="sending"}>
    ...
    <div className={`card${wide ? " span2" : ""}`} onClick={()=>onSelect(p)} style={{position:"relative"}}>
      {p.lazo && (
        <div style={{position:"absolute",top:0,right:0,width:100,height:100,overflow:"hidden",zIndex:10,pointerEvents:"none"}}>
          <div style={{position:"absolute",top:22,right:-28,width:130,background:"var(--purple)",color:"white",fontSize:8,fontWeight:700,letterSpacing:".12em",textTransform:"uppercase",textAlign:"center",padding:"6px 0",transform:"rotate(45deg)",boxShadow:"0 2px 8px rgba(0,0,0,.3)"}}>
            {p.lazo}
          </div>
        </div>
      )}
      <div className="card-img-wrap">
        ...
        {p.destacado && <div style={{position:"absolute",top:42,left:14,background:"#B8860B",color:"white",fontSize:9,letterSpacing:".1em",textTransform:"uppercase",padding:"4px 8px",fontFamily:"DM Sans,sans-serif",fontWeight:600}}>★ Destacada</div>}
      </div>
      <div className="card-body">
        ...
        <div className="card-feats">
          ...
          {p.cochera && <span className="feat">{Ic.car} {p.cochera} {parseInt(p.cochera)===1?"cochera":"cocheras"}</span>}
        </div>
        <div className="card-foot">
          <div className="card-price">{p.precio ? `U$S ${p.precio.replace(/^U\$S\s*/,"")}` : "Consulte"}</div>
...
// ─── Admin field components — outside AdminDrawer to prevent re-mount on keystroke ─
const fldStyle = {width:"100%",padding:"8px 10px",border:"1px solid var(--border)",background:"var(--off)",fontFamily:"DM Sans,sans-serif",fontSize:12,color:"var(--ink)",outline:"none"};
function AdminField({label, k, obj, setObj, type="text", full=false, options=null}) {
  return (
    <div style={{marginBottom:10,gridColumn:full?"1/-1":"auto"}}>
      ...
      {options
        ? <select value={obj[k]||""} onChange={e=>setObj(p=>({...p,[k]:e.target.value}))} style={{...fldStyle,appearance:"none"}}>{options.map(o=><option key={o}>{o}</option>)}</select>
        : type==="textarea"
          ? <textarea value={obj[k]||""} onChange={e=>setObj(p=>({...p,[k]:e.target.value}))} rows={2} style={{...fldStyle,resize:"vertical"}}/>
          : <input type={type} value={obj[k]||""} onChange={e=>setObj(p=>({...p,[k]:e.target.value}))} style={fldStyle}/>
      }
    </div>
  );
}

function AdminInlineForm({obj, setObj, onSave, onCancel, saveLabel}) {
  return (
    <div style={{background:"var(--off)",border:"1px solid var(--border)",padding:"20px 24px",marginBottom:2}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 16px"}}>
        <AdminField label="Tipo" k="tipo" obj={obj} setObj={setObj} options={["Departamento","Casa/Chalet","PH","Terreno","Hotel","Local","Oficina"]}/>
        <AdminField label="Operación" k="etiqueta" obj={obj} setObj={setObj} options={["Venta","Alquiler"]}/>
        <AdminField label="Título" k="titulo" obj={obj} setObj={setObj} full/>
        <AdminField label="Dirección" k="direccion" obj={obj} setObj={setObj}/>
        <AdminField label="Ciudad" k="ciudad" obj={obj} setObj={setObj}/>
        <AdminField label="Precio (ej: U$S 85.000)" k="precio" obj={obj} setObj={setObj}/>
        <AdminField label="Tag (Oportunidad, Apto Crédito…)" k="tag" obj={obj} setObj={setObj}/>
        <AdminField label="m² cubiertos" k="m2" obj={obj} setObj={setObj} type="number"/>
        <AdminField label="m² lote" k="m2lote" obj={obj} setObj={setObj} type="number"/>
        <AdminField label="Dormitorios" k="dorm" obj={obj} setObj={setObj} type="number"/>
        <AdminField label="Baños" k="banos" obj={obj} setObj={setObj} type="number"/>
        <AdminField label="Cocheras" k="cochera" obj={obj} setObj={setObj} type="number"/>
        <AdminField label='Lazo diagonal (ej: "Vendido", "Reservado")' k="lazo" obj={obj} setObj={setObj} full/>
        <div style={{gridColumn:"1/-1",display:"flex",flexDirection:"column",gap:10,padding:"10px 0",borderTop:"1px solid var(--border)",marginTop:4}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div onClick={()=>setObj(p=>({...p,destacado:!p.destacado}))} style={{width:18,height:18,border:`2px solid ${obj.destacado?"#B8860B":"var(--border)"}`,background:obj.destacado?"#B8860B":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,transition:"all .15s",borderRadius:2}}>
              {obj.destacado && <svg width="11" height="11" viewBox="0 0 10 10" fill="none" stroke="white" strokeWidth="2.5"><polyline points="1.5,5 4,7.5 8.5,2"/></svg>}
            </div>
            <div style={{fontSize:12,color:"var(--ink)",fontWeight:500,cursor:"pointer"}} onClick={()=>setObj(p=>({...p,destacado:!p.destacado}))}>
              Propiedad destacada <span style={{fontSize:11,color:"var(--mid)",fontWeight:400}}>— aparece primero con ★</span>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div onClick={()=>setObj(p=>({...p,standby:!p.standby}))} style={{width:18,height:18,border:`2px solid ${obj.standby?"#C0392B":"var(--border)"}`,background:obj.standby?"#C0392B":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,transition:"all .15s",borderRadius:2}}>
              {obj.standby && <svg width="11" height="11" viewBox="0 0 10 10" fill="none" stroke="white" strokeWidth="2.5"><polyline points="1.5,5 4,7.5 8.5,2"/></svg>}
            </div>
            <div style={{fontSize:12,color:"var(--ink)",fontWeight:500,cursor:"pointer"}} onClick={()=>setObj(p=>({...p,standby:!p.standby}))}>
              En standby <span style={{fontSize:11,color:"var(--mid)",fontWeight:400}}>— no visible en el sitio</span>
            </div>
          </div>
        </div>
        <AdminField label="Descripción" k="descripcion" obj={obj} setObj={setObj} type="textarea" full/>
        <AdminField label="URL imagen" k="imagen" obj={obj} setObj={setObj} full/>
      </div>
        ...
        <div className="admin-body">
          ...
          {listings.map((l,i)=>(
            <div key={l.id} draggable onDragStart={()=>onDragStart(i)} onDragOver={e=>onDragOver(e,i)}
              ...
              {editing===l.id
                ? <AdminInlineForm obj={form} setObj={setForm} onSave={saveEdit} onCancel={cancelEdit} saveLabel="Guardar"/>
                : (
                  <div style={{display:"grid",gridTemplateColumns:"8px 56px 1fr auto",gap:12,alignItems:"center",padding:"12px 0",borderBottom:"1px solid var(--border)"}}>
                    ...
                    <div>
                      <div style={{fontSize:13,fontWeight:500,marginBottom:2,lineHeight:1.3,display:"flex",alignItems:"center",gap:8}}>
                        {l.titulo||"(sin título)"}
                        {l.destacado && <span style={{fontSize:9,letterSpacing:".1em",textTransform:"uppercase",background:"#FDF3DC",color:"#B8860B",padding:"2px 7px",borderRadius:2}}>★ destacada</span>}
                        {l.standby && <span style={{fontSize:9,letterSpacing:".1em",textTransform:"uppercase",background:"#FEE",color:"#C0392B",padding:"2px 7px",borderRadius:2}}>standby</span>}
                      </div>
          ...
          {adding
            ? <AdminInlineForm obj={addForm} setObj={setAddForm} onSave={saveAdd} onCancel={()=>setAdding(false)} saveLabel="Agregar propiedad"/>
...
export default function App() {
  ...
  }) : listings).filter(l => !l.standby).sort((a,b) => (b.destacado?1:0)-(a.destacado?1:0));
      ...
      <>
        ...
        <nav className="nav">
          ...
          <ul className="nav-links">
            ...
            <li><a href="#" onClick={e=>{e.preventDefault();setSelected(null);setTimeout(()=>document.getElementById("propiedades")?.scrollIntoView({behavior:"smooth"}),80);}}>Propiedades</a></li>
            <li><a href="#" onClick={e=>{e.preventDefault();setSelected(null);setTimeout(()=>document.getElementById("contacto")?.scrollIntoView({behavior:"smooth"}),80);}}>Contacto</a></li>
          </ul>
    ...
    <>
      ...
      <nav className="nav">
        ...
        <ul className="nav-links">
          ...
          <li><a href="#nosotros" onClick={e=>{e.preventDefault();document.getElementById("nosotros")?.scrollIntoView({behavior:"smooth"})}}>Quiénes somos</a></li>
      ...
      <section className="hero" style={{gridTemplateColumns:"1fr",minHeight:"62vh",overflow:"visible"}}>
        <div className="hero-left" style={{position:"relative",padding:"100px 15vw 80px"}}>
          <div style={{position:"absolute",top:-60,right:380,pointerEvents:"none",overflow:"visible"}}>
          ...
          <p className="hero-sub">Más de 25 años de experiencia en compraventa y alquiler de inmuebles residenciales, turísticos y comerciales en Mar del Plata y la zona.</p>
      ...
      {/* PROPERTIES HEADING + DESTACADAS above filter */}
      <section className="section" id="propiedades" style={{paddingBottom:0}}>
        <div className="sec-head">
          ...
          <div style={{display:"flex",alignItems:"center",gap:20}}>
            {filters && <span style={{fontSize:11,color:"var(--mid)"}}>{visibleListings.filter(l=>!l.destacado).length} resultado{visibleListings.filter(l=>!l.destacado).length!==1?"s":""} · <button onClick={()=>setFilters(null)} style={{border:"none",background:"none",color:"var(--purple)",cursor:"pointer",fontSize:11,padding:0,textDecoration:"underline"}}>limpiar</button></span>}
          </div>
        </div>
        {/* Destacadas always shown above filter */}
        {visibleListings.filter(l=>l.destacado).length > 0 && (
          <div className="prop-grid" style={{marginBottom:0}}>
            {visibleListings.filter(l=>l.destacado).map((p,i)=><Card key={p.id} p={p} wide={i===0} onSelect={setSelected}/>)}
          </div>
        )}
      </section>
      ...
      {/* PROPERTY GRID — non-destacadas below filter */}
      <section className="section" style={{paddingTop:0}}>
        {visibleListings.filter(l=>!l.destacado).length === 0 && !filters
          ? null
          : visibleListings.filter(l=>!l.destacado).length === 0
            ? <div style={{padding:"48px 0",textAlign:"center",color:"var(--mid)",fontSize:14}}>No se encontraron propiedades con estos filtros.</div>
            : <div className="prop-grid">{visibleListings.filter(l=>!l.destacado).map((p,i)=><Card key={p.id} p={p} wide={i===0} onSelect={setSelected}/>)}</div>
        }
      </section>
      ...
      <div className="purple-band">
        ...
        <div className="pb-icons" style={{display:"none"}}>
        ...
        <div className="about">
          ...
          <div style={{position:"relative"}}>
            <div style={{overflow:"hidden",aspectRatio:"4/3"}}>
              <img src="https://inmobiliaticastorage.blob.core.windows.net/imagenes-propiedades/Files/250202/Sliders/2_1_7974175_enc.jpg" alt="Mar del Plata" style={{width:"100%",height:"100%",objectFit:"cover",display:"block",filter:"brightness(.88) saturate(.9)"}}/>
            </div>
        ...
        <div className="ft">
          ...
          <div>
            ...
            <div style={{position:"relative",width:"100%",height:160,overflow:"hidden",border:"1px solid rgba(255,255,255,.08)",marginBottom:10}}>
              <iframe
                title="Street View Oficina"
                width="100%" height="100%"
                style={{border:0,display:"block"}}
                loading="lazy"
                src="https://www.google.com/maps/embed?pb=!4v1700000000001!6m8!1m7!1sCAoSLEFGMVFpcE94WkpNNzFSZlhQV3lZeGhPV0tKbEVYcWlVMUxlQ0F5YU5YZUVM!2m2!1d-38.0021!2d-57.5428!3f180!4f0!5f0.78"
              />
            </div>
